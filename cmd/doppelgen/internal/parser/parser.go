...
const (
	...
	generatedByMarker   = "Code generated by doppelgen" // generatedByMarker is the standard comment placed at the top of every generated file.
)
...
// collectExistingCloneMethods scans all non-generated files for method declarations named Clone
// with a pointer receiver, recording which types already have a Clone implementation.
// Files generated by doppelgen are excluded so that re-running the generator can overwrite
// its own output — the generated Clone() methods should not block regeneration.
func collectExistingCloneMethods(files []*ast.File) map[string]bool {
	result := make(map[string]bool)

	for _, file := range files {
		// Skip doppelgen-generated files — their Clone() methods are meant to be overwritten.
		if isDoppelgenGenerated(file) {
			continue
		}
...
// isDoppelgenGenerated checks whether a file was produced by doppelgen by looking for the
// standard "Code generated by doppelgen" marker comment. This follows the Go convention
// described at https://go.dev/s/generatedcode.
func isDoppelgenGenerated(file *ast.File) bool {
	for _, cg := range file.Comments {
		for _, c := range cg.List {
			if strings.Contains(c.Text, generatedByMarker) {
				return true
			}
		}
	}
