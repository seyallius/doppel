...
The generator produces files like `user.clone_gen.go` with a header
`Code generated by doppelgen. DO NOT EDIT.`

### Generated test files

Alongside each `*.clone_gen.go` file, doppelgen also produces a companion
test file named `*.clone_gen_test.go`. These test files exercise the
generated `Clone()` method with the following checks:

| Test function                      | What it verifies                                                    |
|------------------------------------|---------------------------------------------------------------------|
| `TestT_Clone_Nil`                  | Cloning a nil pointer returns nil without error.                    |
| `TestT_Clone`                      | Round-trip: non-nil input produces a different pointer, no error.   |
| `TestT_Clone_DeepCopyIndependence` | Deep-copied slices, maps, and pointers are independent of original. |
| `TestT_Clone_SkipFields`           | Skip-tagged fields receive their zero value in the clone.           |
| `TestT_Clone_ShallowFields`        | Shallow-tagged containers share the same reference as the original. |
| `TestT_Clone_EmptyFields`          | Empty-tagged containers are non-nil but have length zero.           |

Not all sub-tests are generated for every struct — only the ones relevant
to the fields and directives present. For example, a struct with no skip
fields will not get a `SkipFields` test.

Structs that use the `clone` directive (convention-based functions) receive
a placeholder test instead, since the convention functions must be provided
by the user before `Clone()` can be called. The placeholder includes a
`t.Skip()` call and comments listing the expected function signatures.

Run the generated tests just like any other Go test:
